...
def load_lr_pipeline():
    pipeline = joblib.load('./models/cca_logistic_regression')
    return pipeline

@st.cache_resource(show_spinner='Model loading')
def load_svc_pipeline():
    pipeline = joblib.load('./models/cca_svc')
    return pipeline
...
def select_model():
    ...
    with col1:
        st.selectbox('Select a Model', options= ['Linear Regression', 'SVC'], key= 'selected_model')
    with col2:
        pass
